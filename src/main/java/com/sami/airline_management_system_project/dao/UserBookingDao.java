...
//
//    public UserBookingDao() throws SQLException {
//        this.conn = DataBaseConnector.getConnection();
//    }
//
//    private UserBooking extractFlightFromResultSet(ResultSet rs) throws SQLException {
...
public class UserBookingDao {
    ...
    public List<UserBooking> getUserBookingsByFlightNo(String flightNo) {
        List<UserBooking> userBookings = new ArrayList<>();
        try {
            Connection conn = DataBaseConnector.getConnection();
            PreparedStatement ps = conn.prepareStatement("SELECT * FROM booking_details WHERE flight_no = ?");
            ps.setString(1, flightNo);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
                userBookings.add(extractUserBookingFromResultSet(rs));
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private UserBooking extractUserBookingFromResultSet(ResultSet rs) throws SQLException {
        UserBooking userBooking = new UserBooking();
        userBooking.setUsername(rs.getString("username"));
        userBooking.setFlight_no(rs.getString("flight_no"));
        userBooking.setDate_of_flight(rs.getString("date_of_flight"));
        userBooking.setNo_of_seats(rs.getInt("no_of_seats"));
        userBooking.setTicket_type(rs.getString("ticket_type"));
        userBooking.setAdults(rs.getInt("adults"));
        userBooking.setChild(rs.getInt("child"));
        userBooking.setPnr(rs.getString("pnr"));
        return userBooking;
    }

    public List<UserBooking> getAllBookings(HttpServletRequest request) {
        List<UserBooking> userBookings = new ArrayList<>();
        try {
    ...
    // ... (additional methods for user bookings can be added as needed)
}
